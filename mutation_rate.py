...
mutation_probs = [0.0, 0.1, 0.3, 0.5, 0.7, 0.9]
...
# Run study
for ax, p_mut in zip(axes, mutation_probs):
    F, duration = run_nsga2(prob_mutation=p_mut, seed=42)
...
plt.suptitle("DTLZ2 - NSGA-II with Varying Mutation Probability (p_mut)", fontsize=16)
plt.tight_layout()
plt.show()